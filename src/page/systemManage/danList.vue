<template>
    <div class="contain">
        <div class="header" style="height: 56px;">
            <span style="font-size:18px;">段位说明</span>
        </div>
        <div class="table-content">
            <el-table   :data="tableData"  style="width: 100%;text-align:center"   :border='true'  @selection-change="handleSelectionChange">
                <el-table-column  prop="danname"    label="段位名称"></el-table-column>
                <el-table-column  prop="id"  label="操作" fixed="right" width='300px'>
                      <template slot-scope="scope">
                          <el-button type="primary" size="mini" @click="linkToEdit(scope.row.id,scope.row.danname,scope.row.level,scope.row.nextDanStar)">编辑</el-button>
                      </template>
                  </el-table-column>
            </el-table>
        </div>

        <el-dialog   title="编辑段位"   :visible.sync="dialogVisible"   width="500px"   :before-close="handleClose">
            <div class="dialog-con">
                <el-form :model="information" :rules="rules" ref="information" label-width="100px" class="">
                    <el-form-item label="段位名称:" prop="danname" style="margin-top:25px;">
                        <el-input v-model="information.danname" placeholder=""></el-input>
                    </el-form-item>
                </el-form>
            </div>
...
<script>
    ...
    export default{
        ...
        methods: {
            ...
            getDanList(){
                let url = CONSTANT.URL.DAN.FINDPAGE
                // let dataForm = {
            ...
            onConfirm(){
                ...
                let url = CONSTANT.URL.DAN.UPDATE;
                let data = {
                    id : this.information.id ,
                    danname : this.information.danname ,
                    level : this.information.level ,
                    nextDanStar : this.information.nextDanStar ,
                };
                let param = JSON.stringify(data);
                // let param = data;
                common.requestAjax(url, param, null, res => {
            ...
            linkToEdit(id,danname,level,nextDanStar){
                this.dialogVisible = true;
                this.information.id = id;
                this.information.danname = danname;
                this.information.level = level;
                this.information.nextDanStar = nextDanStar;
            },
