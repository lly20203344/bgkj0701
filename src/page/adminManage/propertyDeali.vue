                            ...
                            <template>  
                              {{item.checkAll}} 
                                ...
                                <el-checkbox-group v-model="role.checkedOrder" >     
                                    <el-checkbox  v-for="(list,num) in item.subMenus" @change="roleListChange(index,num)" :label="list.id" :key="list.id">{{list.menuName}} {{list.id}} </el-checkbox>   
                                </el-checkbox-group> 
                            </template>
                        </div>
          </div>-->
...
export default {
  data() {
    return {
      ...
      rules: {
        ...
        roleList: [{ type: "array", required: true, trigger: "change" }],
        checkedOrder: [
          {
            type: "array",
            message: "请选择角色权限",
  ...
  methods: {
    ...
    onDeali() {
      let url = CONSTANT.URL.ADMIN.Api_find_detail;
      ...
      common.requestAjax(url, param, null, res => {
        if (res.code == 200) {
          ...
          this.menusIds=data.menuIds
          this.role.id=data.id
          var arr = Array.from(data.menuIds);
          this.role.checkedOrder = arr;
          this.getList();
        }
      });
    },
    ...
    handleSave: function(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          let url = CONSTANT.URL.ADMIN.updatePPROLE,
            data,
            param;
